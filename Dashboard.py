...
# Carrega os dados da guia em um DataFrame
...
if aba == "Gráficos":
    ...
    st.plotly_chart(fig_faturamento_produto, use_container_width=True)
  ...
  # Gráfico 1 - Distribuição de Idade por Gênero
    ...
    marcas = df["Indique suas preferências a produtos eletrônicos:"].str.split(", ", expand=True).stack().reset_index(level=1, drop=True)
    ...
    fig_grafico_9.update_layout(width=1200, height=800, title="Distribuição de Dispositivos por Gênero")
    ...
    fig_grafico_10.update_layout( polar=dict(radialaxis=dict(visible=True, showticklabels=False),), showlegend=False,  title="Preferências de Produtos Eletrônicos" )
